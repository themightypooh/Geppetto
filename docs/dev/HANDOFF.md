...
The other docs: [README.md](../../README.md) is what the tools are,
...
[WORK-ORDER.md](WORK-ORDER.md) is what to do next.

---

## Two GitHub remotes

**Work in this repo.** It is `themightypooh/toolshed-dev` (`origin`). Full tree: tests, `tools/`, these docs, probes, `.mcp.json`. Commit and push here as usual.

The public site `themightypooh/Geppetto` (`public`) is a billboard. Do not clone it, do not edit it, do not open PRs against it.

When a chunk of work is done and should be on the internet:

```
python tools/ship.sh
```

Working tree must be clean (commit first). That copies the product files and force-pushes `public/main`. It does not change `origin`.
...
`dotnet` is installed but not on `PATH` in Git Bash, hence the prefix. About 30 seconds, **2099
...
Both tools ship as one s&box library, Geppetto. `` IS the published package;
everything outside it is the host project, the kernel's canonical copy, tests and private work.

```
                     the package — this is what ships
  Code/RigControl/        runtime + asset types (.riganim, .ctrlrig), IK/limit solver
  Code/PixelArmsStyle.cs  the knobs pixel_arms.vmat reads, for the rig viewport
...
Assets/shaders/pixel_arms.shader   the one shader here known to compile — used by the rig viewport

Effigy/                   the CAD kernel — engine-free, the CANONICAL copy
Effigy.Tests/             2532 headless checks

Editor/Probes/            console diagnostics. Private: they read the tools, the tools do not
                          read them, so they stay out of the package
Editor/EffigyTool.cs      the demo-box spike — needs the private Blender pipeline
Editor/HaloMount/, Editor/Pipeline/
                          UNRELATED personal work — do not touch.
                          UNTRACKED and absent from a fresh clone; see below
```

### The two kernel copies

`Effigy/` is canonical. `Editor/Effigy/` is a **mirror** and must never be
hand-edited.

The reason is structural: s&box compiles a library's `Code/` into a game assembly and its `Editor/`
into an editor assembly, and nothing else — a top-level `Effigy/` is invisible to it. The kernel cannot live in
...
- **It reads a real MCC install** (Steam app 976730). Without MCC installed it finds nothing.
  Restore `"Mounts": ["halomcc"]` in `geppetto.sbproj` if you bring this back — the public
  project file ships with an empty mount list on purpose.
