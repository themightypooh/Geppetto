...
**8. The animation clip export.** `File → Animation Clips...`, add a `.riganim` posed against a
model Effigy exported, then `Compile .vmdl`. The editor half compiles and has never been opened.
What to check, in order: the dialog opens and lists nothing; the picker offers `.riganim` assets;
an added clip shows its name, path and looping switch; renaming it sticks; `Compile .vmdl` writes
`anim_<name>.dmx` beside `export.dmx` and the log reports frames, rate and how many bones were
posed. **Then the part no headless check can reach** — load the compiled model and confirm it
carries the clip under that name and that playing it moves the model. A clip that compiles and does
not move is the expected failure and means bone names did not match; the export warns about that
case before writing, so the warning is itself worth provoking once by adding a clip from a
different rig.
...
## 5. Rig Control — known rough edges
...
`.sbproj` is `Type: addon`, `Org: pooh` → ident `pooh.toolshed`. Target Game deliberately empty.

**The consumption path is not proven.** Project Settings → Packages — the screen where another dev
would add `pooh.toolshed` — ships with Facepunch's own warning: *"This stuff hasn't been properly
