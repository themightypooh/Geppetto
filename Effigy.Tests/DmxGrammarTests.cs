...
public static class DmxGrammarTests
{
	...
	static Node FindFirst( Node root, string type ) => FindAll( root, type ).FirstOrDefault();

	static List<Node> FindAll( Node root, string type )
	{
	...
	/// </summary>
	static Node Parse( string text, out string error )
	{
		error = null;
