...
/// and declines; the repair needs per-face loop splitting rather than one whole loop. The same
/// shape arrives a second way: the single-face repair closes a hole by triangulating, so the next
/// cut into that surface lands across a dozen coplanar
...
public static class CurvedHoleTests
{
	...
	/// triangles the first repair left — cutting a body that has already been cut.
	/// </summary>
