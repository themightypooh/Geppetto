...
public static class FaceSketchTests
{
	...
	static void TestUseReference()
	{
		...
		// MID-EDGE TO MID-EDGE, which is the move anyone actually reaches for, and which USE ALONE
		// DOES NOT FINISH.
		//
		// The line's ends sit ON two opposite edges but are not endpoints OF them. ProfileFinder
		// walks half-edges around shared POINT INDICES, so those two points have degree one, the
		// line is pruned as an open chain, and the square stays one region. Nothing about this is
		// specific to a used outline - draw a rectangle and a line across it and the same thing
		// happens - so the missing piece is curve SPLITTING at crossings, not anything here.
		//
		// Pinned rather than left to be rediscovered: this check passing means the gap is still
		// open, and whoever closes it will be sent straight here by it failing.
		...
		Report.Check( "a line across the MIDDLE still does not split it - curves are not cut at crossings",
			crossed.Profiles.Count == 1 && crossed.Warnings.Count == 1,
