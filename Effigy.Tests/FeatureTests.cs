...
public static class FeatureTests
{
	...
	/// <summary>
	/// Subdividing picked faces rather than the whole cage.
	///
	/// The two properties worth asserting are the ones a naive implementation gets wrong: the shape
	/// must not move (linear, not Catmull-Clark, or the untouched half of the box would be dragged
	/// in with the selection), and the mesh must stay watertight — a neighbouring face left alone
	/// would have a T-junction on the edge the selection just split.
	/// </summary>
	static void TestLocalSubdivide()
	{
		var studio = StudioWithBox();
		...
		Check( "picked face became four", mesh.FaceCount == 9, $"{mesh.FaceCount}" );
		Check( "only the picked face's points were added", mesh.VertexCount == 13,
			$"{mesh.VertexCount}" );

		var predicted = CatmullClark.PredictLocalCost( Primitives.Box( 2, 2, 2 ), new[] { 4 }, 1 );
		Check( "local cost prediction is exact",
			predicted.Faces == mesh.FaceCount && predicted.Vertices == mesh.VertexCount,
			$"predicted {predicted.Vertices}v {predicted.Faces}f" );

		// The box is 2 units across and must still be. Catmull-Clark on a subset would have pulled
		// the corners in, which is the whole reason this path is linear.
		var width = mesh.Positions.Max( p => p.x ) - mesh.Positions.Min( p => p.x );
		var height = mesh.Positions.Max( p => p.z ) - mesh.Positions.Min( p => p.z );
		Check( "the shape did not move", Math.Abs( width - 2f ) < 1e-4f && Math.Abs( height - 2f ) < 1e-4f,
			$"{width:0.###} x {height:0.###}" );

		Check( "no T-junction against the neighbours", Watertight( mesh ) );

		// The unselected faces around it grew a corner each rather than being subdivided.
		Check( "neighbours were stitched, not subdivided",
			mesh.Faces.Count( f => f.Count == 5 ) == 4, string.Join( ",", mesh.Faces.Select( f => f.Count ) ) );

		// A second level goes over the same region, not outward into the stitched neighbours.
		subdiv.Levels.Value = 2;
		studio.MarkAllDirty();
		studio.Rebuild();

		var deeper = studio.Bodies[0].Mesh;
		var predictedDeeper = CatmullClark.PredictLocalCost( Primitives.Box( 2, 2, 2 ), new[] { 4 }, 2 );

		Check( "two levels stay watertight", Watertight( deeper ) );
		Check( "two levels cost what was predicted",
			predictedDeeper.Faces == deeper.FaceCount && predictedDeeper.Vertices == deeper.VertexCount,
			$"predicted {predictedDeeper.Vertices}v {predictedDeeper.Faces}f, got {deeper.VertexCount}v {deeper.FaceCount}f" );

		// A pick that no longer resolves is a warning, not a failure: there is still geometry.
		subdiv.Faces.Add( new FaceRef( "body-that-is-gone", new Vec3( 0, 0, 5 ), new Vec3( 0, 0, 1 ) ) );
		studio.MarkAllDirty();
		studio.Rebuild();

		Check( "a lost pick warns rather than fails",
			subdiv.Error is null && subdiv.Warning is not null, subdiv.Error ?? subdiv.Warning ?? "silent" );

		// And with nothing picked it is the whole-body Catmull-Clark it always was.
		subdiv.Faces.Clear();
	...
	static bool Watertight( PolyMesh mesh ) =>
		mesh.BuildEdgeFaces().Values.All( faces => faces.Count == 2 );
