...
public static class DocumentTests
{
	...
	static void TestRoundTrip()
	{
		...
		Report.Check( "and which region of it", extrude.RegionSeeds.Count == 1
			&& MathF.Abs( extrude.RegionSeeds[0].x - 1.5f ) < 1e-6f, $"{extrude.RegionSeed}" );
	...
	static void TestBadInput()
	{
		...
		// Documents written before RegionSeeds was a list stored one point as `vec2 RegionSeed`.
		var oldSeed = "effigy 1\nfeature ExtrudeFeature\n\tid e1\n"
			+ "\ttext SketchFeatureId sketch1\n\tvec2 RegionSeed 1.5 1.5\nend\n";
		var loadedOld = StudioDocument.Read( oldSeed ).Features.Single() as ExtrudeFeature;

		Report.Check( "a document that stored one RegionSeed still loads it as the list",
			loadedOld is not null && loadedOld.RegionSeeds.Count == 1
			&& MathF.Abs( loadedOld.RegionSeeds[0].x - 1.5f ) < 1e-6f
			&& MathF.Abs( loadedOld.RegionSeeds[0].y - 1.5f ) < 1e-6f,
			loadedOld is null ? "did not load" : $"{loadedOld.RegionSeed}" );
