...
public sealed class SketchFeature : Feature
{
	...
	protected override void Execute( FeatureContext ctx )
	{
		...
		// Constraints are design intent; the solver turns them into concrete point positions before
		// anything downstream (extrude, revolve, profiles) reads the sketch. No constraints → no-op.
		if ( Sketch.Constraints.Count > 0 )
		{
			var result = SketchSolver.Solve( Sketch );
			if ( !result.Converged )
			{
				Warning = $"Sketch constraints did not fully converge "
					+ $"(residual {result.Residual:0.####e0} after {result.Iterations} iters). "
					+ "Geometry was left at the best attempt.";
			}
		}
