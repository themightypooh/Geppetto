...
internal sealed class EffigyFeatureDialog : Widget
{
	...
	/// <summary>The picked regions when the dialog opened, for the same reason as the id above: they
	/// are a plain field rather than an IParam, so the generic snapshot cannot see them and an
	...
	/// <summary>Fires when a Sculpt feature's dialog wants the viewport to enter sculpt mode.</summary>
	public Action<SculptFeature> SculptRequested { get; set; }
	...
	public void Open( Feature feature, bool isNew )
	{
		...
		// An existing sketch already has its plane. Opening it IS editing it — the tree's Edit
		// used to stop at this dialog, and a second "Edit sketch" button below the fold was the
		// actual way in. Onshape has no such step: choosing the sketch is entering it, the same
		// way picking a plane on a new one already is.
		if ( _planeChosen && _feature is SketchFeature sketch )
			SketchRequested?.Invoke( sketch );

		// Same for sculpt: the tree's Edit used to park on this dialog, and a second "Sculpt"
		// item in the context menu was the actual way in. A sculpt that already has a cage is
		// ready — opening it is brushing it. A new one with no cage stays here so you can pick
		// a body; OnBodySelectionChanged enters once that cage exists.
		if ( _feature is SculptFeature sculpt && sculpt.Sculpt is not null )
			SculptRequested?.Invoke( sculpt );
	}
	...
	///  - Extrude/Revolve: the profile box arms and the closed regions in the viewport become
	///    hoverable and clickable, however many there are, as long as they share a plane.
	///
	...
	private void ArmPendingSelection( bool isNew )
	{
		...
		{
			// Subdivide is the exception: no picks means the whole body, which is the answer most
	...
	/// had not. Nothing about the feature changes here; the click only repeats the question.
	///
	...
	public bool ReassertPending()
	{
		...
		// waiting, and clicking Sketch means "put me back in it".
		if ( _planeChosen && _feature is SketchFeature sketch )
		{
			SketchRequested?.Invoke( sketch );
			return true;
		...
		// A sculpt with a cage is the same: clicking Sculpt again means "put me back in it"
		// rather than stacking a second sculpt. No cage yet means the body box is still the
		// question, so that falls through to Arm below.
		if ( _feature is SculptFeature sculpt && sculpt.Sculpt is not null )
		{
			SculptRequested?.Invoke( sculpt );
			return true;
	...
	private void RestoreSnapshot()
	{
		if ( _feature is SketchConsumingFeature consumer )
		{
			...
			// Put back unconditionally, unlike the id: an empty list IS a value here - it means
			// every region - so faces picked over an empty selection have to be undone as well as
			// one pick swapped for another. The type check above is what stands in for "a snapshot
			// was taken".
			if ( _regionSeedsSnapshot is not null )
			{
				consumer.RegionSeeds.Clear();
				consumer.RegionSeeds.AddRange( _regionSeedsSnapshot );
			}
		}
	...
	public void Rebuild()
	{
		...
		if ( PickedFaces( _feature ) is { } faces )
		{
			...
			if ( _feature is SubdivideFeature subdivide )
			{
				// Under the Levels field, because it is the answer to the question that field
				// raises. See EffigySubdivideCost: the kernel has always been able to predict this
				// and nothing was asking it.
				AddRow( new EffigySubdivideCost( _body, subdivide, _pickableBodiesLookup ) );
				return;
			}
	...
	/// <summary>A region was picked in the viewport, so the consumer's input changed. Unlike
	/// a plane pick there is no second stage to drop into — the dialog stays open for the
	/// distance and direction parameters, and for the next region: Rebuild would recreate the
	/// selector and drop the armed state a multi-face pick is still using.</summary>
	...
	private Widget BuildBodySelectionRow( BodySelectionParam bs )
	{
		...
		// The dialog tracks one armable so Escape can stand it down. Subdivide has both a face box
		// and a body box; the ??= leaves the face box holding it, which is right — the face box is
		// built first because it is the more specific of the two and the one you came to use.
	...
	private void OnBodySelectionChanged()
	{
		...
		// A new sculpt waits on its body the way a new sketch waits on its plane. RaiseEdited
		// has just rebuilt, so if that pick produced a cage, drop into sculpt mode — picking
		// the body IS entering the sculpt, not a prelude to another button.
		if ( _feature is SculptFeature sculpt && sculpt.Sculpt is not null )
			SculptRequested?.Invoke( sculpt );
	}
}
...
/// selector, but picking closed regions drawn in the viewport instead of a sketch as a whole.
/// This is the box a brand new Extrude opens with, armed and blue, because the profile is the
/// one thing the tool cannot guess.
///
/// Multi-select, and that is the point. A sketch with six closed faces is six answers, not one,
/// and they stay selectable together as long as they share a plane. A click toggles a region
/// and the box stays armed until you dismiss it, the same way the face and body boxes do.
/// </summary>
internal sealed class EffigySketchSelector : Widget, IArmableSelection
{
	...
	private string SelectionLabel()
	{
		...
		var n = _consumer.RegionSeeds.Count;

		if ( n == 0 )
			return $"All regions of {name}";
	...
	protected override void OnPaint()
	{
		...
		Paint.SetPen( Theme.TextLight );
		Paint.SetDefaultFont( 8 );
		Paint.DrawText( label.Shrink( 8f, 2f, 0f, 0f ), "Profiles", TextFlag.LeftTop );

		var box = new Rect( 8f, 18f, Width - 16f, 22f );
		var chosen = SelectionLabel();
	...
	{
		var incoming = _viewport.PickableSketches.FirstOrDefault( s => s.FeatureId == featureId );

		if ( incoming is null )
			return;

		// Clicking a CURVE names no region: every closed region of that sketch, which is also
		// how a face pick is undone back to "the whole sketch".
		...
		var existing = _viewport.PickableSketches.FirstOrDefault( s => s.FeatureId == _consumer.SketchFeatureId );
		var awaiting = string.IsNullOrEmpty( _consumer.SketchFeatureId )
			|| _consumer.SketchFeatureId == SketchConsumingFeature.AwaitingPick
			|| existing is null;

		// First pick, or a click on a different plane: that sketch and this one face.
		if ( awaiting || !SketchArrangement.Coplanar( existing.Sketch.Plane, incoming.Sketch.Plane ) )
		{
			_consumer.SketchFeatureId = featureId;
			_consumer.RegionSeeds.Clear();
			_consumer.RegionSeeds.Add( seed );
			Push();
			Update();
			_changed?.Invoke();
			return;
		...
		// Same plane, possibly a different Sketch feature. Seeds live in the consumed sketch's UV.
		var uv = incoming.FeatureId == existing.FeatureId
			? seed
			: existing.Sketch.Plane.ToPlane( incoming.Sketch.Plane.ToWorld( seed ) );

		// Empty seeds currently mean every region. Clicking a face narrows that to the face.
		if ( _consumer.RegionSeeds.Count == 0 )
		{
			_consumer.RegionSeeds.Add( uv );
			Push();
			Update();
			_changed?.Invoke();
			return;
		...
		for ( var i = _consumer.RegionSeeds.Count - 1; i >= 0; i-- )
		{
			if ( !SamePickedRegion( existing, uv, _consumer.RegionSeeds[i] ) )
				continue;

			_consumer.RegionSeeds.RemoveAt( i );
			Push();
			Update();
			_changed?.Invoke();
			return;
	...
	/// <summary>Two seeds name the same face when some coplanar sketch's smallest containing
	/// region holds both. Compared per sketch so an exclusive face of a neighbour still
	/// toggles, which a host-only profile list would miss.</summary>
	private bool SamePickedRegion( EffigyViewport.PickableSketch host, Vec2 a, Vec2 b )
	{
		foreach ( var pickable in _viewport.PickableSketches )
		{
			if ( !SketchArrangement.Coplanar( host.Sketch.Plane, pickable.Sketch.Plane ) )
				continue;

			var neighbors = _viewport.PickableSketches
				.Where( s => s.FeatureId != pickable.FeatureId )
				.Select( s => s.Sketch );

			var profiles = ProfileFinder.Find( pickable.Sketch, neighbors ).Profiles;
			var auv = pickable.FeatureId == host.FeatureId
				? a
				: pickable.Sketch.Plane.ToPlane( host.Sketch.Plane.ToWorld( a ) );
			var buv = pickable.FeatureId == host.FeatureId
				? b
				: pickable.Sketch.Plane.ToPlane( host.Sketch.Plane.ToWorld( b ) );
			var ra = ProfileFinder.SmallestContaining( profiles, auv );
			var rb = ProfileFinder.SmallestContaining( profiles, buv );

			if ( ra is not null && ReferenceEquals( ra, rb ) )
				return true;
		}
	...
	/// <summary>Keep the viewport's lit set in step with the feature, so what is highlighted is
...
/// <summary>
/// What the subdivide about to happen will cost, under the Levels field, updated as you drag it.
///
/// THIS CONTROL IS WHY PredictCost EXISTS. The kernel comment on it says so outright — "cheap
/// enough to call on every parameter change so a level slider can warn before it makes eight
/// million vertices rather than after" — and then nothing ever called it from the editor. Levels
/// goes to 6, which is 4096x per face, and the only feedback you got was the editor going away.
/// A rebuild is synchronous on the UI thread, so an unaffordable level is not a slow rebuild you
/// can wait out: it is a hang, and export is often where you find out, because export rebuilds.
///
/// It predicts rather than measures, so it is right BEFORE you commit rather than after. Cost is
/// recomputed only when something that feeds it changed — the level, how many faces are picked, or
/// the size of the cage underneath — because the predictor walks the input mesh and a dense one is
/// not free to walk on every repaint.
/// </summary>
internal sealed class EffigySubdivideCost : Widget
{
	private const float RowHeight = 24f;

	/// <summary>Amber, and then red. Neither is a limit — the numbers are the user's business and
	/// there are legitimate reasons to make a very dense mesh. They are the point at which a
	/// sentence about what is going to happen is worth more than a number.</summary>
	private const int Caution = 200_000;
	private const int Danger = 1_000_000;

	private readonly SubdivideFeature _feature;
	private readonly Func<IEnumerable<Body>> _bodies;
	...
	protected override void OnPaint()
	{
		...
		if ( key != _key )
		{
			...
			// The predictor reads the same fields the feature will, so a picked-face subdivide is
			// costed by PredictLocalCost and a whole-body one by PredictCost, without this control
			// having to know which of the two it is looking at.
			_cost = _feature.PredictCost( bodies );
		}

		var severe = _cost.Vertices >= Danger;
		var caution = _cost.Vertices >= Caution;

		var tint = severe ? Theme.Red : caution ? Theme.Yellow : Theme.TextLight.WithAlpha( 0.55f );

		Paint.SetDefaultFont( 8 );
		Paint.SetPen( tint );

		var text = $"{_cost.Vertices:N0} vertices, {_cost.Faces:N0} faces";

		if ( severe )
			text += " — this will lock the editor while it rebuilds";
		else if ( caution )
