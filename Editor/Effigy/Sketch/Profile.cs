...
public sealed class Profile
{
	...
	///
	/// Features that build every region (no <c>RegionSeeds</c>) skip these, or the lens would be
...
///
/// A LINE DRAWN ACROSS A RECTANGLE IS THE SAME PROBLEM FROM THE OTHER SIDE. The integer graph
/// does not see a T-junction: the divider's ends sit on the edges and share no point index with
/// them, so they are pruned as dangling and the rectangle stays one face. Two such lines that
/// cross in the middle are four faces anyone can see and one face the walk reports. The imprint
/// already splits those — T-junctions are proper crossings, not endpoint-on-endpoint kisses —
/// and <see cref="RecoverCutRegions"/> uses that copy when the unsplit walk threw away geometry
/// that the arrangement would have turned into faces. Overlapping closed loops are left alone:
/// those have no dangling curves, and the wholes-plus-lens rule above still applies.
///
...
public static class ProfileFinder
{
	...
	public static ProfileResult Find( Sketch sketch, IEnumerable<Sketch> neighbors )
	{
		...
		RecoverCutRegions( sketch, result );
	...
	/// <summary>
	/// Interior cuts that never joined the integer graph — a line across a rectangle, two lines
	/// crossing in the middle of one — become faces on the imprinted copy. The unsplit walk
	/// prunes them as dangling because coincidence is identity, then reports the outer loop as
	/// one region. That is the right editing model and the wrong picture: the user drew four
	/// panes and got one.
	///
	/// Only runs when the unsplit walk actually discarded open chains. Two overlapping rectangles
	/// have no dangling curves; they stay two wholes plus a lens. A tail hanging off a corner
	/// shares a vertex and does not cross anything, so the imprint is a no-op and the tail stays
	/// reported. And a recovered partition has to cover the same area as the unsplit outers —
	/// otherwise an overlapping pair that happened to have a stray line would lose its wholes.
	/// </summary>
	static void RecoverCutRegions( Sketch sketch, ProfileResult result )
	{
		if ( result.OpenChains == 0 )
			return;
		...
		var recovered = new ProfileResult();
		NestInto( recovered, CollectLoops( working, recovered ) );

		if ( recovered.Profiles.Count <= result.Profiles.Count )
			return;

		if ( result.Profiles.Count > 0 )
		{
			var before = result.Profiles.Sum( p => p.Area );
			var after = recovered.Profiles.Sum( p => p.Area );
			var tol = 1e-3f * MathF.Max( 1f, MathF.Max( before, after ) );

			if ( MathF.Abs( after - before ) > tol )
				return;
		}

		result.Profiles.Clear();
		result.Profiles.AddRange( recovered.Profiles );
		result.Warnings.Clear();
		result.Warnings.AddRange( recovered.Warnings );
	...
	/// <summary>The smallest region that contains <paramref name="seed"/>, or null. Same rule
	/// the viewport pick uses: two overlapping wholes both contain a click in the lens, and
	/// the lens wins because it is smaller.</summary>
	public static Profile SmallestContaining( IEnumerable<Profile> profiles, Vec2 seed )
	{
		Profile picked = null;
	...
	/// <summary>A profile expressed in another plane's coordinates. Identity when the two
	/// planes share origin and axes, so a coplanar neighbour with the same UV is free.</summary>
	public static Profile Project( Profile profile, SketchPlane from, SketchPlane to ) => new()
	{
		Outer = ProjectLoop( profile.Outer, from, to ),
		Holes = profile.Holes.Select( h => ProjectLoop( h, from, to ) ).ToList(),
		FromOverlap = profile.FromOverlap
	};

	static Profile ProjectProfile( Profile profile, SketchPlane from, SketchPlane to ) =>
		Project( profile, from, to );
