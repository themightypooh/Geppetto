...
**Status (September 2026):** early. Rig Control is usable — open it and run the
tutorial. Effigy's kernel is tested (2000+ headless checks); the editor is not.
**Sketch editing is currently broken** (`EnterSketch` never runs). Issues and
PRs welcome; see [CONTRIBUTING.md](CONTRIBUTING.md).

For what is actually finished, see [docs/dev/WHAT-IS-BUILT.md](docs/dev/WHAT-IS-BUILT.md).
For what is not, see [docs/dev/WHAT-IS-LEFT.md](docs/dev/WHAT-IS-LEFT.md).
...
You can open `marionette.sbproj` in s&box and use this project as-is.

**2. Or copy the folders you want into your own s&box project.** Each tool is
independent.
...
s&box can reference other packages — Project Settings → Packages — and in
principle you'd add `pooh.marionette` there and be done. As of writing, that
...
It's also unclear whether a referenced package's `Editor/` code compiles into
the consuming project's editor assembly at all, which is precisely what an
editor tool needs. So: copy the folders. It works today.
...
makes the whole thing testable without an engine in front of you, and 2000+
...
`.effigy` is the document — hand-written text, one record per line, diffable,
with fields found by reflection so a new feature saves the moment it is written.
Export writes DMX (what ModelDoc actually imports) or OBJ for static geometry,
plus a hand-written KV3 `.vmdl` beside it.
