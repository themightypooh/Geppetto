...
/// <summary>
/// Persistent storage form of a constraint. Holds the kind plus the point/curve indices and any
/// scalar payload (e.g. Distance value). <see cref="Build"/> produces the runtime
/// <see cref="IConstraint"/> the solver evaluates. Keeping storage separate from evaluation means
/// the disk/clone format stays a simple struct while the solver stays generic.
/// </summary>
public sealed class SketchConstraint
{
	public SketchConstraintKind Kind;
	/// <summary>Legacy: curve id for the old H/V-on-curve form. Prefer PointA/PointB.</summary>
	...
	/// <summary>Build the runtime evaluator. Returns null if the constraint cannot be resolved
	/// (e.g. Horizontal/Vertical still stored by CurveId only and the curve is missing).</summary>
	public IConstraint Build( Sketch sketch )
	{
		switch ( Kind )
		{
			case SketchConstraintKind.Coincident:
				return new CoincidentConstraint( PointA, PointB );
			case SketchConstraintKind.Distance:
				return new DistanceConstraint( PointA, PointB, Value );
			case SketchConstraintKind.Horizontal:
				if ( PointA >= 0 && PointB >= 0 )
					return new HorizontalConstraint( PointA, PointB );
				return ResolveLineConstraint( sketch, true );
			case SketchConstraintKind.Vertical:
				if ( PointA >= 0 && PointB >= 0 )
					return new VerticalConstraint( PointA, PointB );
				return ResolveLineConstraint( sketch, false );
			case SketchConstraintKind.EqualLength:
				return new EqualLengthConstraint( PointA, PointB, PointC, PointD );
			case SketchConstraintKind.Parallel:
				return new ParallelConstraint( PointA, PointB, PointC, PointD );
			case SketchConstraintKind.Perpendicular:
				return new PerpendicularConstraint( PointA, PointB, PointC, PointD );
			default:
				return null;
		}
	}
	...
	IConstraint ResolveLineConstraint( Sketch sketch, bool horizontal )
	{
		...
		if ( curve is not SketchLine line ) return null;
		return horizontal
...
///
/// Constraints are evaluated by <see cref="SketchSolver"/>. Point indices are shared across
/// curves so coincidence is identity; the solver derives positions from the constraint set
/// rather than requiring typed coordinates for every corner.
/// </summary>
public sealed class Sketch
{
	...
	/// <summary>Legacy H/V constraint attached to a curve id (resolved at solve time).</summary>
	...
	/// <summary>Point-pair constraint (Coincident, Distance, Horizontal, Vertical on two indices).</summary>
	public SketchConstraint AddConstraint( SketchConstraintKind kind, int a, int b, float value = 0f )
	{
		var c = new SketchConstraint( kind, a, b, value );
		Constraints.Add( c );
		return c;
	}

	/// <summary>Four-point constraint (Equal length, parallel, perpendicular between two segments).</summary>
	public SketchConstraint AddConstraint( SketchConstraintKind kind, int a0, int a1, int b0, int b1 )
	{
		var c = new SketchConstraint( kind, a0, a1, b0, b1 );
		Constraints.Add( c );
		return c;
	}
	...
	public Sketch Clone() => new()
	{
		...
		Constraints = Constraints.Select( CloneConstraint ).ToList(),
		Tolerance = Tolerance
	};

	static SketchConstraint CloneConstraint( SketchConstraint c )
	{
		return new SketchConstraint( c.Kind, c.CurveId )
		{
			PointA = c.PointA,
			PointB = c.PointB,
			PointC = c.PointC,
